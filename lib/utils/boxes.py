...
def flipud_boxes(boxes, im_height):
    """Flip boxes updown."""
    boxes_flipped = boxes.copy()
    boxes_flipped[:, 1::4] = im_height - boxes[:, 3::4] - 1
    boxes_flipped[:, 3::4] = im_height - boxes[:, 1::4] - 1
    return boxes_flipped


def rot90_boxes(boxes, im_width, im_heigth, k):
    """Rotation boxes 90*k degree"""
    assert k == 1 or k == 2 or k == 3
    if k == 1:
        boxes_roted = boxes.copy()
        boxes_roted[:, 0::4] = boxes[:, 1::4]
        boxes_roted[:, 2::4] = boxes[:, 3::4]
        boxes_roted[:, 1::4] = im_heigth - boxes[:, 2::4] - 1
        boxes_roted[:, 3::4] = im_heigth - boxes[:, 0::4] - 1

    elif k == 2:
        boxes_roted = boxes.copy()
        boxes_roted[:, 0::4] = im_width - boxes[:, 2::4] - 1
        boxes_roted[:, 2::4] = im_width - boxes[:, 0::4] - 1
        boxes_roted[:, 1::4] = im_heigth - boxes[:, 3::4] - 1
        boxes_roted[:, 3::4] = im_heigth - boxes[:, 1::4] - 1

    elif k == 3:
        boxes_roted = boxes.copy()
        boxes_roted[:, 1::4] = boxes[:, 0::4]
        boxes_roted[:, 3::4] = boxes[:, 2::4]
        boxes_roted[:, 0::4] = im_width - boxes[:, 3::4] - 1
        boxes_roted[:, 2::4] = im_width - boxes[:, 1::4] - 1
    return boxes_roted
