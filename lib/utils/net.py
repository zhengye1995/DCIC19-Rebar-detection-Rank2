...
def iou_loss(bbox_pred, bbox_targets):
    ...
    AoG = torch.abs(bbox_targets[2] - bbox_targets[0] + 1) * torch.abs(bbox_targets[3] - bbox_targets[1] + 1)
    AoP = torch.abs(bbox_pred[2] - bbox_pred[0] + 1) * torch.abs(bbox_pred[3] - bbox_pred[1] + 1)

    # overlaps are the co-ordinates of intersection box
    overlap_0 = torch.max(bbox_targets[0], bbox_pred[0])
    overlap_1 = torch.max(bbox_targets[1], bbox_pred[1])
    overlap_2 = torch.max(bbox_targets[2], bbox_pred[2])
    overlap_3 = torch.max(bbox_targets[3], bbox_pred[3])
    ...
    N = iou_loss.size(0)  # batch size
    loss_box = iou_loss.view(-1).sum(0) / N
