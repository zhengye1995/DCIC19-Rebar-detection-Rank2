...
def im_detect_bbox_aug(model, im, box_proposals=None):
    ...
    # Perform detection on the vertical flipped image
    if cfg.TEST.BBOX_AUG.V_FLIP:
        scores_vf, boxes_vf, _ = im_detect_bbox_vflip(
            model,
            im,
            cfg.TEST.SCALE,
            cfg.TEST.MAX_SIZE,
        ...
        add_preds_t(scores_vf, boxes_vf)

    # Perform detection on the rotation90 image
    if cfg.TEST.BBOX_AUG.ROTATION90:
        for k in range(1, 4):
            scores_90, boxes_90, _ = im_detect_bbox_roation90(
                model,
                im,
                cfg.TEST.SCALE,
                cfg.TEST.MAX_SIZE,
            ...
            add_preds_t(scores_90, boxes_90)
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        if cfg.TEST.BBOX_AUG.SCALE_V_FLIP:
            scores_scl_vf, boxes_scl_vf = im_detect_bbox_scale(
                model, im, scale, max_size, box_proposals, vflip=True
            )
            add_preds_t(scores_scl_vf, boxes_scl_vf)
        if cfg.TEST.BBOX_AUG.SCALE_ROTATION90:
            for k in range(1, 4):
                scores_scl_90, boxes_scl_90 = im_detect_bbox_scale(
                    model, im, scale, max_size, box_proposals, rotation90=True, k=k
                )
                add_preds_t(scores_scl_90, boxes_scl_90)
...
def im_detect_bbox_vflip(
        model, im, target_scale, target_max_size, box_proposals=None):
    """Performs bbox detection on the vertical flipped image.
    Function signature is the same as for im_detect_bbox.
    """
    # Compute predictions on the flipped image
    im_vf = im[::-1, :, :]
    ...
    if not cfg.MODEL.FASTER_RCNN:
        box_proposals_vf = box_utils.flipud_boxes(box_proposals, im_height)
    else:
        box_proposals_vf = None

    scores_vf, boxes_vf, im_scale, _ = im_detect_bbox(
        model, im_vf, target_scale, target_max_size, boxes=box_proposals_vf
    )

    # Invert the detections computed on the flipped image
    boxes_inv = box_utils.flipud_boxes(boxes_vf, im_height)

    return scores_vf, boxes_inv, im_scale


def im_detect_bbox_roation90(
        model, im, target_scale, target_max_size, box_proposals=None, k=1):
    """Performs bbox detection on the rotation90 image.
    Function signature is the same as for im_detect_bbox.
    """

    # Compute predictions on the rotation image
    im_90 = box_utils.rotate_image(im, 90*k)

    im_height = im.shape[0]
    im_weight = im.shape[1]

    if not cfg.MODEL.FASTER_RCNN:
        box_proposals_90 = box_utils.rot90_boxes(box_proposals, im_weight, im_height, k)
    else:
        box_proposals_90 = None

    scores_90, boxes_90, im_scale, _ = im_detect_bbox(
        model, im_90, target_scale, target_max_size, boxes=box_proposals_90
    )

    # Invert the detections computed on the flipped image
    boxes_inv = box_utils.rot90_boxes(boxes_90, im_weight, im_height, k)

    return scores_90, boxes_inv, im_scale


def im_detect_bbox_scale(
        model, im, target_scale, target_max_size, box_proposals=None, hflip=False, vflip=False, rotation90=False, k=1):
    ...
    if hflip:
        ...
        scores_scl, boxes_scl, _ = im_detect_bbox_vflip(
            model, im, target_scale, target_max_size, box_proposals=box_proposals
        )
    elif rotation90:
        scores_scl, boxes_scl, _ = im_detect_bbox_roation90(
            model, im, target_scale, target_max_size, box_proposals=box_proposals, k=k
        )
    else:
...
def box_results_with_adaption_nms_and_limit(scores, boxes, imgname):  # NOTE: support single-batch
    """Returns bounding-box detection results by thresholding on scores and
    applying non-maximum suppression (NMS).
    ...
    for j in range(1, num_classes):
        ...
        boxes_j = boxes[inds, j * 4:(j + 1) * 4]
        dets_j = np.hstack((boxes_j, scores_j[:, np.newaxis])).astype(np.float32, copy=False)
        bboxes_counts = np.load("bboxes_count.npy").item()
        if bboxes_counts[imgname] > 90:
            nms_threash = 0.4  # 钢筋多的图应该遮挡严重，所以nms阈值给的大一点，抑制弱一点
        else:
            nms_threash = 0.3
        keep = box_utils.nms(dets_j, nms_threash)
        nms_dets = dets_j[keep, :]
        # Refine the post-NMS boxes using bounding-box voting
        if cfg.TEST.BBOX_VOTE.ENABLED:
            nms_dets = box_utils.box_voting(
